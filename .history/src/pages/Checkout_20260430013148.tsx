...
import { calculateCarTotalPrice, calculateRentalDuration } from '../lib/pricingUtils';
...
const CheckoutForm = ({ amount, bookingData }: any) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (bookingData.type === 'flight' && bookingData.passengers && bookingData.passengerNames) {
       const missingNames = bookingData.passengers.some((_: any, i: number) => !bookingData.passengerNames[i] || bookingData.passengerNames[i].trim() === '');
       if (missingNames) {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/bookings', {
        ...
        body: JSON.stringify({
          ...
          rooms: bookingData.type === 'hotel' ? (bookingData.rooms || []) : undefined,
          nights: bookingData.durationDays || 1,
          totalAmount: amount,
          paymentIntentId: fakePaymentIntentId,
          passengers: bookingData.type === 'flight' ? bookingData.passengers.map((p: any, i: number) => ({
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || 'Failed to create booking on backend.');
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {bookingData.type === 'flight' && bookingData.passengers && (
        <div className="space-y-4 pt-4 border-t border-border mt-4">
          <h4 className="font-bold text-sm uppercase px-1">Passenger Details</h4>
          {bookingData.passengers.map((p: any, i: number) => (
            <Input
              key={i}
              placeholder={`Passenger ${i + 1} Name (${p.type})`}
              value={bookingData.passengerNames[i]}
              onChange={e => bookingData.onPassengerNameChange(i, e.target.value)}
...
export default function Checkout() {
  ...
  const [resolvedBookingData, setResolvedBookingData] = useState<any>(stateBookingData);
  ...
  const { formatPrice } = useCurrency();
  const [passengerNames, setPassengerNames] = useState<string[]>(Array.from({ length: adults + children + infants }).fill('') as string[]);
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        ...
        let endpoint = type === 'hotel' ? `/api/hotels/${id}` : type === 'flight' ? `/api/flights/${id}` : `/api/cars/${id}`;
        ...
        if (res.ok) {
           ...
           if (type === 'hotel' && roomsParam) {
              try {
                ...
                const availableRooms = [...data.rooms].filter(r => r.inventory && r.inventory.availableRooms > 0);
                ...
                for (const reqRoom of parsedRooms) {
                   const matchingType = availableRooms.find((rt: any) => {
                       ...
                       return rt.capacity && rt.capacity.maxAdults >= (reqRoom.adults || 1) && 
  ...
  if (loadError) return (
    <div className="container mx-auto p-20 text-center flex flex-col items-center gap-6">
      <p className="font-black uppercase tracking-widest text-destructive">Checkout Gateway Timeout.</p>
      <p className="text-muted-foreground text-sm font-semibold">We couldn't prepare the secure gateway. Please try again.</p>
      <Button onClick={() => navigate(-1)} className="rounded-xl font-bold uppercase tracking-widest">Go Back</Button>
    </div>
  );
  ...
  if (actualType === 'car') {
     ...
     if (pickupDate && dropoffDate) {
        const days = calculateRentalDuration(new Date(pickupDate), new Date(dropoffDate));
        console.log(`[Car Checkout] pickupDate=${pickupDate}, dropoffDate=${dropoffDate}, duration=${days}`);
        ...
        basePrice = resolvedBookingData.totalPrice || calculateCarTotalPrice(item.pricePerDay, days);
        durationText = `(x${days} days)`;
     } else {
     ...
     durationText = resolvedBookingData?.durationDays ? `(x${resolvedBookingData.durationDays} nights)` : `(x1 nights)`;
  } else {
     // Flight
     flightPricing = calculateExactUserFormula(item.price, { adults, children, infants }, cabinClass, tripType);
  ...
  if(actualType === 'flight' && flightPricing) surcharge = flightPricing.taxes / 100;

  const totalAmount = actualType === 'flight' && flightPricing ? flightPricing.total / 100 : basePrice + surcharge;

  if (actualType === 'hotel') {
    console.log({
       basePricePerNight: hotelPricingBreakdown ? (hotelPricingBreakdown.basePrice / hotelPricingBreakdown.roomsCount / (resolvedBookingData?.durationDays || 1)) : item.pricePerNight,
       nights: resolvedBookingData?.durationDays || 1,
      ...
      <div className="w-full lg:w-1/2 space-y-10">
        ...
        <Card className="rounded-[40px] border-none shadow-2xl bg-card overflow-hidden">
           <div className="aspect-[21/9] relative overflow-hidden bg-muted">
             {actualType === 'hotel' ? (
                <SafeImage src={item.images?.[0]} className="w-full h-full object-cover" />
             ) : actualType === 'car' ? (
                <SafeImage 
                ...
                <SafeImage src={getFlightImage(item.arrivalCity, item.image, item._id)} className="w-full h-full object-cover" />
             )}
             <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
             <div className="absolute bottom-10 left-10">
                <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter">{actualType === 'hotel' ? item.name : actualType === 'car' ? `${item.brand} ${item.model}` : item.airline}</h2>
                <p className="text-white/60 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                   {actualType === 'hotel' || actualType === 'car' ? <MapPin size={14} /> : <Plane size={14} />}
                   {actualType === 'hotel' || actualType === 'car' ? `${item.city}, ${item.country}` : `${item.departureCity} → ${item.arrivalCity}`}
                </p>
             </div>
          </div>
             ...
             <div className="grid grid-cols-2 gap-10">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 font-black italic uppercase">
                    ...
                    <span>{actualType === 'hotel' || actualType === 'car' ? `${item.rating || 5} Stars` : `${item.class} Elite`}</span>
                  </div>
                </div>
                <div className="space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block italic">Reference</span>
                  <p className="font-black italic uppercase italic">{actualType === 'hotel' ? 'Luxe Property' : actualType === 'car' ? 'Vehicle Rental' : item.flightNumber}</p>
                </div>
             </div>
             ...
             <div className="pt-8 border-t border-border/50">
               ...
               <div className="space-y-4">
                  {actualType === 'flight' && flightPricing ? (
                    <>
                      {adults > 0 && <div className="flex justify-between items-center text-sm italic font-medium"><span className="text-muted-foreground">Adults (x{adults})</span><span>{formatPrice(flightPricing.adultTotal / 100)}</span></div>}
                      {children > 0 && <div className="flex justify-between items-center text-sm italic font-medium"><span className="text-muted-foreground">Children (x{children})</span><span>{formatPrice(flightPricing.childTotal / 100)}</span></div>}
                      {infants > 0 && <div className="flex justify-between items-center text-sm italic font-medium"><span className="text-muted-foreground">Infants (x{infants})</span><span>{formatPrice(flightPricing.infantTotal / 100)}</span></div>}
                    ...
                    <>
                      <div className="flex justify-between items-center text-sm italic font-medium">
                        ...
                        <span className="font-bold">{hotelPricingBreakdown?.totalGuests || 1} Guests / {hotelPricingBreakdown?.roomsCount || 1} Rooms</span>
                      </div>
                      <div className="flex justify-between items-center text-sm italic font-medium pt-2 border-t border-border">
                        <span className="text-muted-foreground">Base Price {durationText}</span>
                        <span>{formatPrice(hotelPricingBreakdown?.basePrice || item.pricePerNight * (resolvedBookingData?.durationDays || 1))}</span>
                      </div>
                      {(hotelPricingBreakdown?.extraGuestsPrice > 0) && (
                        <div className="flex justify-between items-center text-sm italic font-medium">
                          <span className="text-muted-foreground">Extra Guests Price {durationText}</span>
                          <span>{formatPrice(hotelPricingBreakdown.extraGuestsPrice)}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center text-sm italic font-medium">
                        <span className="text-muted-foreground">Premium Surcharge & Taxes (15%)</span>
                        <span>{formatPrice(surcharge)}</span>
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="flex justify-between items-center text-sm italic font-medium">
                        <span className="text-muted-foreground">
                          {actualType === 'car' ? `Total for ${durationText.match(/\d+/) ? durationText.match(/\d+/)?.[0] : 1} days` : `Standard Rate ${durationText}`}
                          {actualType === 'car' && <span className="block text-xs mt-1">{formatPrice(item.pricePerDay)} / day</span>}
                        </span>
                  ...
                  <div className="pt-4 flex justify-between items-center">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-4xl font-black italic tracking-tighter text-primary underline underline-offset-8 decoration-4 leading-none">{formatPrice(totalAmount)}</span>
                      {actualType === 'car' && <span className="text-xs font-bold text-muted-foreground mt-2 italic uppercase">total</span>}
                    </div>
                  </div>
               </div>
             </div>
      ...
      <div className="w-full lg:w-1/2 mt-10 lg:mt-32">
        <Card className="rounded-[40px] shadow-[0_48px_96px_-12px_rgba(0,0,0,0.14)] dark:shadow-[0_48px_96px_-12px_rgba(0,0,0,0.5)] border-none overflow-hidden bg-card">
          ...
          <CardContent className="p-12 pt-0">
            <CheckoutForm amount={totalAmount} bookingData={{ 
              type: actualType, id: actualId, item, 
              rooms: resolvedBookingData?.rooms || [],
              durationDays: resolvedBookingData?.durationDays || 1,
              passengers: actualType === 'flight' ? [
