      ...
      <section className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="container relative z-10 mx-auto px-4 text-center">
          ...
          >
            ...
            <p className="text-white/80 text-xl max-w-3xl mx-auto italic drop-shadow-lg">{location.state?.description || data.description}</p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            ...
            <div className="space-y-10">
              ...
              <div className="grid gap-6">
                {data.hotels && data.hotels.length > 0 ? data.hotels.slice(0, 3).map((hotel: any) => (
                  <Card key={hotel._id} className="overflow-hidden border-none shadow-xl bg-muted/20 hover:bg-muted/30 transition-colors rounded-3xl group cursor-pointer" onClick={() => navigate(`/checkout/hotel/${hotel._id}`)}>
                    <CardContent className="p-0 flex h-40">
                      ...
                      <div className="p-6 flex-1 flex flex-col justify-center">
                        ...
                        <p className="text-xs text-muted-foreground uppercase font-bold tracking-widest mb-4">{hotel.address || hotel.city}</p>
                        <p className="text-primary font-black italic">From {formatPrice(hotel.pricePerNight)} <span className="text-muted-foreground text-[10px] uppercase">/ night</span></p>
                      </div>
                    </CardContent>
                  </Card>
            ...
            <div className="space-y-10">
              ...
              <div className="grid gap-6">
                {data.flights && data.flights.length > 0 ? data.flights.slice(0, 3).map((flight: any) => (
                  <Card key={flight._id} className="overflow-hidden border-none shadow-xl bg-card rounded-[30px] transition-all hover:shadow-2xl cursor-pointer relative h-48 group" onClick={() => navigate(`/checkout/flight/${flight._id}`)}>
                    <img 
                    ...
                    <div className="absolute top-4 left-4 z-10">
                      <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-xl flex items-center gap-2 border border-white/10 shadow-lg">
                        {flight.airlineLogo ? (
                          <img src={flight.airlineLogo} className="w-4 h-4 object-contain filter invert" alt={flight.airline} />
                        ) : (
                          <Plane className="w-3 h-3 text-white/70" />
                        )}
                        <span className="text-white text-[10px] font-bold tracking-widest uppercase">{flight.airline}</span>
                      </div>
                    </div>

                    <div className="absolute bottom-0 left-0 right-0 p-5 flex justify-between items-end z-10">
                      <div>
                        <h3 className="text-white text-2xl font-black uppercase tracking-tighter italic leading-none mb-1">{flight.departureAirport} → {flight.arrivalAirport}</h3>
                        <div className="flex items-center gap-1.5 text-white/80 text-[10px] font-bold uppercase tracking-widest">
                           <Clock size={10} className="text-primary hidden sm:inline" />
                      ...
                      <div className="text-right">
                        <p className="text-white text-2xl font-black italic tracking-tighter leading-none">{formatPrice(flight.price / 100)}</p>
                      </div>
                    </div>
                  </Card>
