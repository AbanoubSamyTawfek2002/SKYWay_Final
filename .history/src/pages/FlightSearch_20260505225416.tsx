...
export default function FlightSearch() {
    ...
    async (isLoadMore = false) => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/flights?${params.toString()}`,
        );
  ...
  const activeFlightData = activeFlightId
    ? flights.find((f) => f._id === activeFlightId)
    : null;

  const activeRoute =
    routeCategories.find((r) => r.from === from && r.to === to)?.name ||
    "All Routes";
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] overflow-hidden lg:flex-row">
      ...
      <div
        id="flight-map-section"
        className="w-full h-[300px] lg:h-full shrink-0 lg:w-[40%] xl:w-[35%] relative border-b lg:border-b-0 lg:border-r border-border bg-muted/10 z-10"
      >
        <FlightsMap
      ...
      <div className="w-full flex-1 lg:w-[60%] xl:w-[65%] overflow-y-auto px-4 sm:px-6 py-6 pb-24 lg:pb-6 custom-scrollbar bg-background">
        <div className="mb-8">
          <span className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary font-black uppercase tracking-widest text-[10px] mb-2 italic">
            SkyWay Elite Board
          </span>
            ...
            <AnimatePresence mode="popLayout">
              {flights.map((flight, i) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className={`overflow-hidden border-none shadow-md bg-card rounded-[30px] transition-all hover:shadow-xl cursor-pointer relative h-72 sm:h-80 group ${activeFlightId === flight._id ? "ring-4 ring-primary" : ""}`}
                    >
                      <SafeImage
                        src={getFlightImage(
                          flight.arrivalCity,
                      ...
                      <div className="absolute top-4 right-4 z-10">
                        <Badge
                          variant="secondary"
                          className="bg-white/20 backdrop-blur-md border-none text-white text-[10px] font-black uppercase tracking-widest px-3 py-1"
                        >
                          {tripType}
                        </Badge>
                      </div>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-5 flex justify-between items-end z-10">
                        <div>
                          <h3 className="text-white text-3xl font-black uppercase tracking-tighter italic leading-none mb-2">
                            {flight.arrivalCity}
                          </h3>
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="mt-2 text-[10px] text-white/70 font-bold uppercase tracking-widest">
                              Adult:{" "}
                              {formatPrice(pricing.adultPricePerPerson / 100)}{" "}
                              (Per Person)
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="text-right flex flex-col items-end">
                          <span className="text-[10px] text-white/70 uppercase font-black tracking-widest mb-1">
                            {cabinClass} Class
                          </span>

                          <div className="text-[10px] text-white/80 uppercase font-bold tracking-widest mb-1">
                            Total for {adults + children + infants} Passenger
                            {adults + children + infants > 1 ? "s" : ""}
                          </div>
                          <span className="text-white text-2xl sm:text-3xl font-black tracking-tighter leading-none">
            ...
            {flights.length === 0 && (
              <div className="py-20 text-center flex flex-col items-center border border-dashed rounded-3xl mt-4">
                ...
                <p className="text-muted-foreground italic text-xs mb-4">
                  Adjust your search criteria to find available routes.
                </p>
                <Button
                  variant="outline"
                  size="sm"
