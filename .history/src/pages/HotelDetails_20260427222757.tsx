...
import { MapPin, Star, Wifi, Droplets, Dumbbell, Coffee, Utensils, Sparkles, User, Calendar, MessageSquare, Send, ShieldCheck, Plane, GlassWater, Plus, Minus, Check } from 'lucide-react';
...
export default function HotelDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const queryStr = roomsParamsStr ? `?rooms=${encodeURIComponent(roomsParamsStr)}` : '';
        const hotelRes = await fetch(`/api/hotels/${id}${queryStr}`).then(res => res.json());
        setHotel(hotelRes);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const pricing = useMemo(() => {
    if (!hotel) return { totalPerNight: 0, subtotal: 0, tax: 0, grandTotal: 0, breakdown: {} };
  ...
  const navigateToCheckout = () => {
    ...
    const availableRooms = [...hotel.rooms].filter(r => r.inventory && r.inventory.availableRooms > 0);
  ...
  const updateRoomCount = (index: number, field: 'adults' | 'children', delta: number) => {
    const newRooms = [...rooms];
    const newValue = Math.max(field === 'adults' ? 1 : 0, (newRooms[index][field] || 0) + delta);
        ...
        <div className="lg:col-span-8 space-y-12 sm:space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-4 h-auto md:h-[500px]">
            ...
            <div className="flex md:flex-col gap-4 sm:gap-4 col-span-1 md:col-span-1 overflow-x-auto md:overflow-visible pb-2 md:pb-0 hide-scrollbar">
              {images.slice(1, 4).map((img: string, i: number) => (
                <div key={i} className="flex-none w-32 md:w-full h-32 md:flex-1 rounded-[16px] overflow-hidden shadow-md group relative">
                  <SafeImage src={img || images[0]} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  {i === 2 && images.length > 4 && (
          ...
          <div className="space-y-6 sm:space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-primary font-black text-xl sm:text-2xl italic leading-none">
                  ...
                  <span className="text-muted-foreground text-[10px] sm:text-xs font-black uppercase tracking-widest not-italic opacity-40 ml-2 sm:ml-4">{hotel.reviewCount || 0} GLOBAL REVIEWS</span>
                </div>
              </div>
            ...
            <div className="max-w-4xl">
              <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter uppercase italic leading-[0.9] mb-4 sm:mb-6">{hotel.name}</h1>
              <div className="flex items-center gap-2 sm:gap-3 text-muted-foreground text-lg sm:text-xl md:text-2xl font-black italic">
                ...
                <span className="opacity-80">{hotel.address ? `${hotel.address}, ` : ''}{hotel.city}, {hotel.country}</span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-12 sm:pt-16 border-t border-border/50">
            <h3 className="text-2xl sm:text-3xl font-black uppercase italic tracking-tighter mb-8 sm:mb-10">Curated Amenities</h3>
              ...
              {hotel.amenities?.map((amenity: string) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground text-center line-clamp-1">{amenity}</span>
                  </motion.div>
                );
              })}
            </div>
          ...
          <section className="pt-12 sm:pt-16 border-t border-border/50">
            <div className="mb-8">
               ...
               <h3 className="text-2xl sm:text-3xl font-black uppercase italic tracking-tighter">Guest Testimonials</h3>
            </div>
            <ReviewSection 
              targetType="hotel" 
              targetId={id!} 
            ...
            <CardContent className="p-6 sm:p-10 space-y-8 sm:space-y-10">
                  ...
                  <div className="flex justify-between items-start">
                     <div className="w-full">
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground italic opacity-60">
                          TOTAL RATE ({numberOfNights} NIGHT{numberOfNights !== 1 ? 's' : ''})
                        </span>
                        <div className="flex flex-col gap-1 mt-2">
                          <div className="flex justify-between items-baseline">
                            ...
                            <span className="text-xs font-black uppercase tracking-widest opacity-40 italic">/ NIGHT</span>
                          </div>
                          ...
                          <div className="space-y-1 pt-4 border-t border-border/10">
                            ...
                            <div className="flex justify-between text-base font-black text-foreground uppercase tracking-tighter pt-2 italic">
                              ...
                              <span className="text-primary">{formatPrice(pricing.grandTotal)}</span>
                            </div>
                          </div>
                        </div>
                     </div>
              ...
              <div className="space-y-4">
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h4 className="font-black uppercase italic tracking-tighter">Your Setup ({rooms.length} Rooms)</h4>
                    <Button variant="ghost" size="sm" className="h-7 px-2 text-[10px] font-black uppercase tracking-widest" onClick={addRoom}>
                  ...
                  {rooms.map((room, idx) => {
                    ...
                    const childCost = room.children * base * 0.70;
                    const roomTotal = base + extraAdultsCost + childCost;
                    
                    return (
                      <div key={idx} className="p-4 border border-primary/20 rounded-[20px] bg-primary/5 space-y-4 relative group">
                        <div className="flex justify-between items-center">
                          <h5 className="font-black text-[10px] uppercase italic tracking-widest text-primary">Room {idx + 1}</h5>
                          {rooms.length > 1 && (
                            <button onClick={() => removeRoom(idx)} className="text-muted-foreground hover:text-destructive transition-colors">
                              <Minus size={14} />
                            </button>
                          )}
                        </div>
                        ...
                        <div className="flex gap-4">
                          <div className="flex-1 space-y-2">
                            <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block text-center">Adults</span>
                            <div className="flex items-center justify-between bg-background rounded-xl p-1 border border-border/50 shadow-sm">
                              <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg" onClick={() => updateRoomCount(idx, 'adults', -1)}>
                                <Minus size={12} />
                              </Button>
                              <span className="text-sm font-black">{room.adults}</span>
                              <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg" onClick={() => updateRoomCount(idx, 'adults', 1)}>
                                <Plus size={12} />
                              </Button>
                            </div>
                          ...
                          <div className="flex-1 space-y-2">
                            <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block text-center">Children</span>
                            <div className="flex items-center justify-between bg-background rounded-xl p-1 border border-border/50 shadow-sm">
                              <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg" onClick={() => updateRoomCount(idx, 'children', -1)}>
                                <Minus size={12} />
                              </Button>
                              <span className="text-sm font-black">{room.children}</span>
                              <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg" onClick={() => updateRoomCount(idx, 'children', 1)}>
                                <Plus size={12} />
                              </Button>
                            </div>
                  ...
                  <div className="pt-2">
                     <Button variant="outline" size="sm" className="w-full text-[10px] font-black uppercase tracking-widest rounded-full h-10" onClick={() => navigate('/')}>Return to Search</Button>
                  </div>
                </div>
                
                <h4 className="font-black uppercase italic tracking-tighter mt-6">Property Room Types</h4>
                <div className="space-y-3">
                  {hotel.rooms?.map((roomType: any) => (
                    <div key={roomType._id} className="p-3 border border-border/50 rounded-[16px] bg-muted/10 space-y-2">
                       <div className="flex justify-between items-start">
                         <div>
                           <h5 className="font-black text-sm uppercase italic">{roomType.name}</h5>
                           <p className="text-[10px] text-muted-foreground">Max {roomType.capacity.maxTotalGuests} Guests ({roomType.capacity.maxAdults} Adults)</p>
                         </div>
                       </div>
                       <p className="text-xs font-bold text-primary">{formatPrice(roomType.pricing.pricePerNight)} Base</p>
                    </div>
                  ))}
                  {(!hotel.rooms || hotel.rooms.length === 0) && (
                    <p className="text-sm italic text-muted-foreground">Loading inventory...</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-2 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 italic">
                  <ShieldCheck size={14} className="text-primary" /> Transparent Pricing, No Hidden Fees
                </div>
              </div>
            </CardContent>
          </Card>
