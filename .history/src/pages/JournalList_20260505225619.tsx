...
export default function JournalList() {
  ...
  const fetchJournals = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/journals?${params.toString()}`,
      );
      if (res.ok) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      >
        <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter mb-4 font-serif italic">
          Travel <span className="text-primary text-stroke">Journal</span>
        </h1>
        <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
          Discover extraordinary destinations and get insider tips for your next
          adventure.
        </p>
      </motion.div>
              ...
              <div className="p-6 flex flex-col flex-1">
                <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-widest font-bold mb-3">
                  <span className="flex items-center gap-1">
                    <MapPin size={12} className="text-primary" /> {journal.city}
                    , {journal.country}
                  </span>
                  <span>•</span>
                  <span>{journal.readTime}</span>
                </div>
                <h3 className="text-2xl font-black mb-3 leading-tight group-hover:text-primary transition-colors font-serif italic">
                  {journal.title}
                </h3>
                ...
                <div className="mt-auto flex items-center justify-between border-t pt-4">
                  ...
                  <Link to={`/journals/${journal._id}`}>
                    <Button
                      variant="ghost"
                      className="uppercase tracking-widest text-xs font-black p-0 h-auto hover:bg-transparent hover:text-primary gap-1"
                    >
