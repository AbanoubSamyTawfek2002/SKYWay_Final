...
import { Plane, Hotel, Calendar, MapPin, CheckCircle2, User, Heart, Settings, XCircle, Car, LogOut } from 'lucide-react';
          ...
          <Card className="border-none shadow-sm bg-muted/30">
            <CardContent className="p-4 flex flex-col gap-2">
              <div className="flex flex-col items-center mb-8 mt-4">
                ...
                {!user?.isVerified && <Badge variant="destructive" className="mt-2 text-[10px]">Unverified</Badge>}
              </div>
              ...
              >
                ...
                {loadingBookings ? (
                  ...
                  <Card className="p-12 text-center text-muted-foreground bg-muted/20 border-dashed">
                    ...
                    <p className="text-xl font-bold mb-2">No trips found yet.</p>
                    <p>Start exploring and book your first adventure today!</p>
                  </Card>
                ) : (
                  <div className="grid gap-4">
                    {bookings.map((booking) => (
                      <Card key={booking._id} className={`overflow-hidden border-l-4 ${booking.status === 'cancelled' ? 'border-l-destructive opacity-75' : 'border-l-primary'}`}>
                        <CardContent className="p-6 flex flex-col lg:flex-row items-center justify-between gap-6">
                          <div className="flex items-center gap-6 flex-1 w-full">
                            <div className={`shrink-0 w-16 h-16 rounded-2xl flex items-center justify-center ${booking.status === 'cancelled' ? 'bg-destructive/10 text-destructive' : 'bg-primary/10 text-primary'}`}>
                              {booking.type === 'flight' ? <Plane size={32} /> : booking.type === 'car' ? <Car size={32}/> : <Hotel size={32} />}
                            </div>
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-sm text-muted-foreground mt-2">
                                <div className="flex items-center gap-1">
                                  <MapPin size={14} /> 
                                  {booking.type === 'hotel' ? (booking.hotelId?.city || 'Location') : booking.type === 'car' ? (booking.carId?.city || 'Location') : (booking.flightId?.airline || 'Airline')}
                                </div>
                                <div className="flex items-center gap-1">
                                  <Calendar size={14} /> 
                                  {booking.type === 'flight' ? (booking.flightId?.departureTime ? new Date(booking.flightId.departureTime).toLocaleDateString() : '') : 'Flexible Dates'}
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-col lg:items-end w-full lg:w-auto gap-3 border-t lg:border-t-0 pt-4 lg:pt-0">
                            <div className="lg:text-right flex justify-between lg:block">
                              ...
                              <p className="text-2xl font-black">{formatPrice(booking.totalAmount)}</p>
                            </div>
                            <div className="flex items-center gap-2 justify-between lg:justify-end">
                               {booking.status === 'cancelled' ? (
                                  ...
                                  <>
                                    ...
                                    <Button variant="outline" size="sm" className="text-destructive border-destructive/30 hover:bg-destructive hover:text-white" onClick={() => handleCancelBooking(booking._id)}>
                                      Cancel
                                    </Button>
                                  </>
                               )}
