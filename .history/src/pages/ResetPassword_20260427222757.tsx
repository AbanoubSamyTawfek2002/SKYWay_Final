    ...
    return (
      <div className="container mx-auto flex items-center justify-center min-h-[calc(100vh-64px)] px-4 py-8">
        <Card className="w-full max-w-md shadow-2xl text-center py-10">
          ...
          <CardTitle className="text-2xl font-black uppercase tracking-tighter mb-2">
            Password Updated!
          </CardTitle>
          <CardDescription className="text-base">
            Your password has been changed successfully. Redirecting to log
            in...
          </CardDescription>
        </Card>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="pt-1">
                <div className="flex justify-between items-center mb-1">
                  ...
                  {strength.label && (
                    <span
                      className={`text-[10px] uppercase font-black ${strength.color.replace("bg-", "text-")}`}
                    >
                      {strength.label}
                    </span>
                  )}
                ...
                <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden flex gap-1">
                  <div
                    className={`h-full flex-1 ${password.length > 0 ? strength.color : "bg-transparent"}`}
                  ></div>
                  <div
                    className={`h-full flex-1 ${strength.label === "Medium" || strength.label === "Strong" ? strength.color : "bg-transparent"}`}
                  ></div>
                  <div
                    className={`h-full flex-1 ${strength.label === "Strong" ? strength.color : "bg-transparent"}`}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {confirmPassword && password !== confirmPassword && (
                <p className="text-[10px] text-destructive font-semibold mt-1 uppercase">
