...
export default function TicketDetails() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(
        `skyway-ticket-${booking.type}-${booking.bookingReference || booking._id}.pdf`,
      );
    } catch (error) {
  ...
  const getAirlineLogo = (airline: string) => {
    const logos: Record<string, string> = {
      ...
      EgyptAir:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/6/63/Egyptair.svg/200px-Egyptair.svg.png",
      "Nile Air":
        "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9e/Nile_Air_logo.svg/200px-Nile_Air_logo.svg.png",
      FlyDubai:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4b/Flydubai_logo.svg/200px-Flydubai_logo.svg.png",
      "Qatar Airways":
        "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b3/Qatar_Airways_logo.svg/200px-Qatar_Airways_logo.svg.png",
      Saudia:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Saudia_logo.svg/200px-Saudia_logo.svg.png",
    };
  ...
  const AirlineLogo = ({
    ...
    if (error || !logoSrc) {
      return (
        <div
          className={`rounded-2xl bg-white/20 flex items-center justify-center font-black italic p-2 ${className}`}
        >
    ...
    return (
      <div
        className={`rounded-2xl bg-white/20 flex items-center justify-center p-2 ${className}`}
      >
        <img
          src={logoSrc}
          alt={airline}
  ...
  if (!booking) {
    return (
      <div className="container mx-auto px-4 py-32 text-center min-h-[70vh]">
        <h1 className="text-3xl font-black uppercase tracking-widest italic text-destructive">
          Booking Not Found
        </h1>
        <Button
          onClick={() => navigate("/my-bookings")}
          className="mt-8 rounded-xl font-bold uppercase tracking-widest h-14 px-8"
        >
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="flex flex-col items-center max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-700 fade-in zoom-in-95">
        <div className="flex justify-between w-full items-end border-b border-border/50 pb-6">
          <div>
            <span className="inline-flex bg-primary/10 text-primary px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-4">
              Official E-Ticket
            </span>
          ...
          <div className="flex gap-4">
            <Button
              variant="outline"
              onClick={() => navigate("/my-bookings")}
              className="rounded-xl font-bold uppercase tracking-widest h-12 shadow-sm border-2"
            >
              Back
            </Button>
            <Button
              ...
              className="rounded-xl font-black uppercase tracking-widest h-12 shadow-xl hover:shadow-primary/20 bg-black text-white hover:bg-black/80"
            >
              {downloading ? (
        ...
        {type === "flight" && item && booking.passengers?.length > 0 ? (
          <div className="w-full flex gap-8 flex-col">
            {booking.passengers.map((passenger: any, pIndex: number) => (
              <Card
                key={pIndex}
                className="w-full border-none shadow-2xl bg-background/50 backdrop-blur rounded-[40px] overflow-hidden"
              >
                <div className="bg-primary p-8 flex justify-between items-center text-primary-foreground">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold opacity-80 mt-1 uppercase tracking-widest">
                        Flight {item.flightNumber || "XY123"}
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    ...
                    <QRCodeSVG
                      value={`https://skyway.com/verify/${passenger.ticketNumber || booking.bookingReference}`}
                      size={64}
                ...
                <div className="p-10 space-y-8">
                  <div className="flex justify-between items-center text-center">
                    <div className="w-1/3 text-left">
                      <p className="text-5xl font-black uppercase tracking-tighter italic leading-none text-primary">
                        {item.from?.substring(0, 3).toUpperCase()}
                      </p>
                      <p className="text-lg font-bold text-muted-foreground mt-2">
                        {item.from}
                      </p>
                      <p className="text-sm font-black mt-2">
                        {formattedDate(item.departureTime || "10:00 AM")}
                      </p>
                    </div>
                    ...
                    <div className="w-1/3 text-right">
                      <p className="text-5xl font-black uppercase tracking-tighter italic leading-none text-primary">
                        {item.to?.substring(0, 3).toUpperCase()}
                      </p>
                      <p className="text-lg font-bold text-muted-foreground mt-2">
                        {item.to}
                      </p>
                      <p className="text-sm font-black mt-2">
                        {formattedDate(item.arrivalTime || "02:00 PM")}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-red-50 text-red-600 p-5 rounded-2xl border border-red-100 flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-red-800">
                        Please arrive at the airport at least 4 hours before
                        departure. Identification is required at boarding.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <Card className="w-full border-none shadow-2xl bg-background/50 backdrop-blur rounded-[40px] overflow-hidden">
            {type === "flight" && item && (
              <div>
                <div className="bg-primary p-8 flex justify-between items-center text-primary-foreground">
                  ...
                  <QRCodeSVG
                    value={`https://skyway.com/verify/${booking.bookingReference || booking._id}`}
                    size={64}
                    className="bg-white p-1 rounded-lg"
                  />
                </div>
              ...
              <div>
                <div className="bg-primary p-8 flex justify-between items-center text-primary-foreground relative overflow-hidden">
                  ...
                  <QRCodeSVG
                    value={`https://skyway.com/verify/${booking.bookingReference || booking._id}`}
                    size={64}
                    className="bg-white p-1 rounded-lg relative z-10"
                  />
                </div>
                ...
                <div className="p-10 space-y-8">
                        ...
                        {formattedDate(
                          new Date(
                            new Date(booking.createdAt).getTime() +
                              (booking.nights || 1) * 24 * 60 * 60 * 1000,
                          ),
                        )}
                      </p>
                      <p className="text-sm text-primary font-bold">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-muted/20 p-5 rounded-2xl border border-border/50">
                      ...
                      <p className="font-black text-lg">
                        {booking.totalGuests || 1} Guest
                        {(booking.totalGuests || 1) > 1 ? "s" : ""}
                      </p>
                    </div>
                    <div className="bg-muted/20 p-5 rounded-2xl border border-border/50">
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">
                ...
                <div className="p-10 space-y-8">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="bg-muted/30 p-6 rounded-3xl border border-border/50">
                      <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-2 flex flex-center gap-2">
            ...
            <div className="bg-muted/50 p-10 border-t border-border/50 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="text-center md:text-right">
                ...
                <p className="font-black text-4xl italic text-primary">
