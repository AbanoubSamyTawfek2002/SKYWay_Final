    ...
    <div className="container mx-auto px-4 py-12 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          ...
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter uppercase italic leading-none italic">
            My Wishlist
          </h1>
        </div>
      </div>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground italic">
                  Start saving your dream flights and hotels today!
                </p>
              </div>
              <Button
                onClick={() => navigate("/")}
                className="rounded-full px-10 h-14 font-black uppercase tracking-widest gap-2"
              >
                  ...
                  <div className="aspect-[16/10] relative overflow-hidden">
                    {item.itemType === "flight" ? (
                      <div className="w-full h-full relative">
                        <img
                          src={getFlightImage(
                            item.details?.arrivalCity,
                            item.details?.image,
                            item.itemId,
                          )}
                          alt={item.details?.arrivalCity}
                        ...
                        <div className="absolute bottom-4 left-4 right-4 flex items-center gap-2">
                          {item.details?.airlineLogo ? (
                            <div className="bg-white p-1 rounded">
                              <img
                                src={item.details?.airlineLogo}
                                alt={item.details?.airline}
                          ...
                          <span className="text-white font-bold text-sm leading-tight drop-shadow-md">
                            {item.details?.airline || item.details?.airline}
                          </span>
                        </div>
                      </div>
                    ) : (
                      <img
                        src={
                          item.itemType === "hotel"
                            ...
                            : "https://images.unsplash.com/photo-1436491865332-7a61a109ce05?auto=format&fit=crop&w=800&q=80"
                        }
                  ...
                  <CardContent className="p-8 flex flex-col flex-1">
                    <div className="mb-6">
                      <h3 className="text-2xl font-black uppercase italic tracking-tighter leading-tight mb-2">
                        {item.itemType === "hotel"
                          ? item.details?.name
                          : `${item.details?.airline || item.details?.airline} (${item.details?.flightNumber})`}
                      </h3>
                    ...
                    <div className="mt-auto space-y-4">
                      <div className="flex justify-between items-end pb-4 border-b border-border/50">
                        <div>
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground italic mb-1">
                            Price
                          </p>
                        ...
                        <Badge
                          variant="outline"
                          className="rounded-full font-black italic border-primary/20 text-primary"
                        >
                          {item.itemType === "hotel"
                            ? `${item.details?.rating} ★`
                            : item.details?.class}
