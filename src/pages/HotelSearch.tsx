...
import { MapPin, Star, Hotel as HotelIcon, ChevronDown, List, Map as MapIcon } from 'lucide-react';
...
export default function HotelSearch() {
  ...
  const searchPricingConfig = useMemo(() => {
    ...
    const guests = rooms.reduce((acc, r) => acc + (r.adults || 1) + (r.children || 0), 0) || 1;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] overflow-hidden lg:flex-row">
      ...
      <div id="hotel-map-section" className="w-full h-[300px] shrink-0 lg:h-full lg:w-[40%] xl:w-[35%] relative border-b lg:border-b-0 lg:border-r border-border bg-muted/10 z-10">
      ...
      <div className="w-full flex-1 lg:w-[60%] xl:w-[65%] overflow-y-auto px-4 sm:px-6 py-6 pb-24 lg:pb-6 custom-scrollbar bg-background">
        <div className="mb-8">
          <span className="inline-block bg-primary/10 text-primary font-black uppercase tracking-widest text-[10px] mb-2 px-3 py-1 rounded-full italic">Find Your Stay</span>
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter uppercase italic leading-none mb-4">Luxe Stays</h1>
          <p className="text-muted-foreground text-sm italic">Explore our curated selection of properties visually.</p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col gap-6">
            {[...Array(4)].map((_, i) => <SkeletonHotelCard key={i} />)}
          </div>
        ) : (
          <>
            <div className="flex flex-col gap-6">
              <AnimatePresence mode="popLayout">
                {hotels.map((hotel, i) => {
                  ...
                  <motion.div 
                    ...
                    className="group"
                  >
                    <Card className={`relative flex flex-col sm:flex-row border border-border/50 bg-card overflow-hidden rounded-[20px] shadow-sm transition-all duration-300 hover:shadow-md cursor-pointer ${activeHotelId === hotel._id ? 'ring-2 ring-primary border-primary' : ''}`}>
                      <div className="relative w-full sm:w-[220px] h-[200px] shrink-0 overflow-hidden">
                        ...
                        <div className="absolute top-2 left-2 z-10 transition-transform duration-300 group-hover:scale-105">
                          <div className="bg-[rgba(0,0,0,0.05)] text-[#111] dark:bg-[rgba(255,255,255,0.08)] dark:border-[rgba(255,255,255,0.15)] dark:text-[#FFFFFF] backdrop-blur-[6px] border border-transparent dark:border-solid px-[12px] py-[6px] rounded-[10px] text-xs font-black uppercase tracking-tighter italic [text-shadow:0_2px_10px_rgba(255,255,255,0.8)] dark:[text-shadow:0_2px_10px_rgba(0,0,0,0.8)] group-hover:shadow-[0_0_15px_rgba(255,255,255,0.2)] flex items-end gap-1">
                            ...
                            <span className="text-[8px] font-bold opacity-80 mb-0.5">/ {searchPricingConfig.nights} NIGHT{searchPricingConfig.nights !== 1 ? 'S' : ''}</span>
                          </div>
                        </div>
                        <div className="absolute top-2 right-2 z-10" onClick={(e) => e.stopPropagation()}>
                          <WishlistButton itemType="hotel" itemId={hotel._id} className="h-8 w-8 bg-black/20 backdrop-blur border-none text-white hover:bg-black/40 scale-75" />
                        </div>
                      </div>
                      ...
                      <CardContent className="p-4 sm:p-5 flex flex-col flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-2 gap-2">
                          <div className="min-w-0">
                            <h3 className="font-black text-lg tracking-tight uppercase italic leading-none mb-1.5 truncate group-hover:text-primary transition-colors cursor-pointer" onClick={(e) => { e.stopPropagation(); navigate(`/hotels/${hotel._id}`); }}>{hotel.name}</h3>
                          ...
                          <div className="flex items-center gap-1 bg-primary/10 text-primary px-1.5 py-0.5 rounded shrink-0">
                            ...
                            <span className="font-black text-xs italic">{hotel.rating?.toFixed(1)}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-auto pt-4 flex items-center justify-between gap-3">
                          <Button 
                            ...
                            onClick={(e) => { e.stopPropagation(); navigate(`/hotels/${hotel._id}?rooms=${searchParams.get('rooms') || ''}&checkIn=${searchParams.get('checkIn') || ''}&checkOut=${searchParams.get('checkOut') || ''}`); }}
                          >
                            Details
                          </Button>
                          <Button 
                            ...
                            onClick={(e) => { e.stopPropagation(); navigate(`/checkout/hotel/${hotel._id}?rooms=${searchParams.get('rooms') || ''}&checkIn=${searchParams.get('checkIn') || ''}&checkOut=${searchParams.get('checkOut') || ''}`); }}
                          >
                            Book
                          </Button>
                        </div>
            ...
            {hotels.length === 0 && (
              <div className="py-20 text-center flex flex-col items-center border border-dashed rounded-3xl mt-4">
                <HotelIcon size={40} className="text-muted-foreground/30 mb-4" />
                <h2 className="text-xl font-black uppercase italic mb-2">No Stays Found</h2>
                <p className="text-muted-foreground italic text-xs mb-4">Try adjusting your filters or destination.</p>
                <Button variant="outline" size="sm" className="rounded-full font-bold uppercase tracking-widest text-[9px]" onClick={() => handleCategorySelect('All')}>View All</Button>
              </div>
            )}
          </>
        )}
